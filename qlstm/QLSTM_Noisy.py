...
class QLSTM(nn.Module):
    def __init__(
        ...
        self.backend = backend  # "default.qubit", "qiskit.basicaer", "qiskit.ibmq"
        ...
        backend = "ibm_nazca"
        noise_model = NoiseModel.from_backend(provider.get_backend(backend))

        self.dev_forget = qml.device(
            self.backend, wires=self.wires_forget, noise_model=noise_model
        )
        self.dev_input = qml.device(
            self.backend, wires=self.wires_input, noise_model=noise_model
        )
        self.dev_update = qml.device(
            self.backend, wires=self.wires_update, noise_model=noise_model
        )
        self.dev_output = qml.device(
            self.backend, wires=self.wires_output, noise_model=noise_model
        )
